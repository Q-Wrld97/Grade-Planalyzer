function addNewData(){
  ...
  auth.onAuthStateChanged(async function(user) {
    if (user) {
      alert(weightScale)
     await db.collection('users').doc(user.uid).collection(semester).doc(courseName).delete()
      // User is signed in.      
     ...
     await db.collection('users').doc(user.uid).update({
      ...
      // now we have to add a sub collection per category
        ...
        for (i=0; i < SubCollectionName.length; i++){
          ...
          await  db.collection('users').doc(user.uid).collection(semester).doc(courseName).collection(SubCollectionName[i]).doc(SubCollectionName[i]+'Grades').delete()
            //add sub collection
          ...
          await  db.collection('users').doc(user.uid).collection(semester).doc(courseName).collection(SubCollectionName[i]).doc(SubCollectionName[i]+'Weight').delete()
          ...
          await  db.collection('users').doc(user.uid).collection(semester).doc(courseName).collection(SubCollectionName[i]).doc(SubCollectionName[i]+'Complete').delete()
