  ...
  auth.onAuthStateChanged(async function (user) {
    if (user) {
      ...
      if (courseData.exists) {
        ...
        //grabbing per cateogry data from DB
        for (i = 0; i < categoryType.length; i++) {
          var courseDataPerCategory = await db
            .collection("users")
            .doc(userUid)
            .collection(semesterTemp)
            .doc(courseNameTemp)
            .collection(categoryType[i])
            .doc(categoryType[i] + "Complete")
            .get();

          if (courseDataPerCategory.exists) {
            completionNameTemp.push(courseDataPerCategory.data());
          }
        }
        console.log(completionNameTemp);
      }
    } else {
